...
public class Paging {
	...
	public Paging(int totalRecord, int nowPage, int numPerPage, int numPerBlock, String url) {
		...
		if(nowBlock > 1) { // nowBlock이 1보다 크다면 이전 페이지의 링크를 구현
			prev.append("<button type='button' class='btn btn-primary' onclick='location=\"")
			.append(url).append("?nowPage=").append((nowBlock - 2) * numPerBlock + 1)
			.append("\"'>이전</button>");
		} else {
		...
		for(int i=0; i<numPerBlock; i++) {
			...
			if(printPage == nowPage) { // 현재 페이지일 경우
				...
				paging.append("<button type='button' class='btn btn-primary' id='btn_nowPage' style='background-color: lightblue;'>")
				...
				.append(url).append("?nowPage=").append(printPage).append("\"'>")
				.append(printPage).append("</button>");
			}
		...
		if(totalBlock > nowBlock) {
			...
			.append(url).append("?nowPage=").append(nowBlock*numPerBlock+1)
			.append("\"'>다음</button>");
		} else {
